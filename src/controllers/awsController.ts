...
import { scaffHoldDetailsById } from "../schemas/tradesManSchema";
...
export class awsCredentialController {
    ...
    async scaffHoldPdf(req: Request, res: Response, next: Function) {
        try {
            const data = scaffHoldDetailsById.parse(req.query); // ✅ Zod validation
            const urlData = await awsData.scaffHoldPdf(data);
            res.status(200).json(urlData);
        } catch (err) {
            next(err);
        }
