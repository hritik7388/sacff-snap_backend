...
import {
  ...
  GetJobCraftDTO,
  getRequestCreatorById,
  GetUserDetailsDTO,
  ImageDTO,
  LoginProjectManagerDTO,
  requestedScaffoldsDTO,
  SearchScaffHoldDTO,
  uploadImageDTO,
} from "../schemas/projectManagerSchema";
...
export class ProjectManagerServices {
  async getDashboardStats(projectManagerId: number) {
    try {
      const [totalRequests, totalProjects, pendingRequests, activeRequests] =
        await Promise.all([
    ...
    try {
      ...
      if (existingProjectManager.isDeleted) {
        throw new CustomError(
          ...
          403,
          "User DELETED by Subadmin",
        );
      }
      const isPasswordValid = await bcrypt.compare(
        data.password,
        existingProjectManager.password,
      );
      if (!isPasswordValid) {
        throw new CustomError(
          RESPONSE_MESSAGES.PROJECTMANAGER.INVALID_PASSWORD,
          500,
          "Invalid password",
        );
      }
      ...
      throw new CustomError(
        RESPONSE_MESSAGES.USER.LOGIN_FAILE,
        500,
  ...
  async getUserDetails(id: number) {
    try {
      ...
      const cmpId =
        user.projectManager?.cmpId || user.competentPerson?.cmpId || null;
      const { userMedias, projectManager, competentPerson, ...rest } = user;
      const idProofImage =
        user.userMedias.find((media) => media.mediaType === "ID_PROOF_IMAGE")
          ?.url || null;
      const photoImage =
        user.userMedias.find((media) => media.mediaType === "PHOTO_IMAGE")
          ?.url || null;
      return {
    ...
    try {
      ...
      const updatedRequest = await prisma.projectScaffholdRequest.update({
        ...
        data: {
          status: data.status,
          reajectionReason:
            data.status === "REJECTED" ? (data.reajectionReason ?? null) : null,
        },
        include: {
          createdBy: {
      ...
      if (isModifiedRequest) {
        ...
        notificationMessage = `Your modification request for Project ${requestData.project.projectName} has been ${data.status}.`;
        ...
        notificationMessage = `Your scaffold request for Project ${requestData.project.projectName} has been ${data.status}.`;
  ...
  async generateProjectJobLink(projectId: string) {
    try {
      ...
      throw new CustomError(
        error.message || "Failed to generate project job link",
